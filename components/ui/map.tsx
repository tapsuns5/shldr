...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    // MapLibre only auto-resizes on window resize. Responsive containers
    // (flex, vh/clamp heights, breakpoints) change size without a window
    // event, so observe the container and call resize() to keep the canvas
    // in sync with its box.
  ...
  return (
    <MapContext.Provider value={contextValue}>
      <div
        ref={containerRef}
        className={cn("relative h-full w-full max-w-full min-w-0 overflow-hidden [&_.maplibregl-canvas]:!max-w-full [&_.maplibregl-map]:!max-w-full", className)}
      >
